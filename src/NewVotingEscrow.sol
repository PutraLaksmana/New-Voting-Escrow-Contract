...
/**
@title New Voting Escrow Contract
@author Putra Laksmana
@license MIT
...
contract NewVotingEscrow is Ownable, ReentrancyGuard {
    ...
    constructor( string memory _name, string memory _symbol, string memory _version, uint8 _decimals, address token_addr, uint min_time) {
        name = _name;
        symbol = _symbol;
        version = _version;
        decimals = _decimals;
    ...
    function deposit_for(address _addr, uint _value, uint unlock_time, LockedBalance memory locked_balance, DepositType deposit_type) internal {
    ...
    function create_lock(uint _value, uint _unlock_time) internal {
        ...
        deposit_for(msg.sender, _value, unlock_time, _locked, DepositType.CREATE_LOCK_TYPE);
    }

    /// @notice External function for create_lock
    /// @param _value Amount to deposit
    /// @param _unlock_time Epoch time when tokens unlock, rounded down to whole weeks
    function CreateLock(uint _value, uint _unlock_time) external nonReentrant onlyUserOrWhitelist notUnlocked {
        create_lock(_value, _unlock_time);
    }
    ...
    function _increase_amount(uint _value) internal {
        ...
        deposit_for(msg.sender, _value, 0, _locked, DepositType.INCREASE_LOCK_AMOUNT);
    }
    ...
    function _increase_unlock_time(uint _unlock_time) internal {
        ...
        deposit_for(msg.sender, 0, unlock_time, _locked, DepositType.INCREASE_UNLOCK_TIME);
    }
    ...
    /// @param _value Amount to deposit
    /// @param _unlock_time Epoch time when tokens unlock, rounded down to whole weeks
    function withdraw_andcreate_lock(uint _value, uint _unlock_time) external nonReentrant onlyUserOrWhitelist notUnlocked {
        _withdraw();
        create_lock(_value, _unlock_time);
    }
